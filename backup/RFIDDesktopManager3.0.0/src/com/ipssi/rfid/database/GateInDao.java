...
//    public static int createTPRStep(Connection conn, TPStep mBean) throws IOException {
...
//        String query = "INSERT INTO tp_step (in_time, out_time, tpr_id, user_by,vehicle_id,"
...
//        return tpr_step_id;
//    }
    /*public static int createTPRStep(Connection conn, TPStep mBean) throws IOException, SQLException {
        int tpr_step_id = Misc.getUndefInt();
        RFIDMasterDao.insert(conn, mBean);
        tpr_step_id = mBean.getId();
        return tpr_step_id;
    }*/
...
//        boolean destroyIt = false;
//        try {
//            ps = conn.prepareStatement("select id from vehicle where name like " + "'" + stdName + "'" + " and status = 1");
//            rs = ps.executeQuery();
//            while (rs.next()) {
...
//        //int tpr_id = Misc.getUndefInt() ;
//        String query = " select vehicle_id from vehicle_extended where vehicle_id = ? and extended_status in (1,100)";
...
//        Connection conn = null;
    ...
    public static void InsertIntoTable(Connection conn, VehicleExtended vehicleExtentedBean) throws Exception {// Any Table
        PreparedStatement ps = null;
        ResultSet rs = null;
        java.sql.Date now = new java.sql.Date((new java.util.Date())
                .getTime());
    ...
    public static boolean updateDriverDetail(Connection invConn, BioMatricBean biomatric_bean) throws Exception{
    ...
    String UPDATE_DRIVER_DETAIL = "UPDATE driver_details set type = ? ,"
            ...
            + " driver_photo = ? ,info4 = ?, "
            + " info3 = ? , lov_field2 = ?, lov_field3 = ?,driver_std_name = ?,driver_dl_number_std_name = ? where id = ?";
    try {
        Timestamp dlExpiryDate = null ;
        java.util.Date date = new java.util.Date();
        Timestamp now = new Timestamp(date.getTime());
        if(biomatric_bean.getDlExpiryDate() != null){
         dlExpiryDate = new Timestamp(biomatric_bean.getDlExpiryDate().getTime());
        }
        Timestamp dob = new Timestamp(biomatric_bean.getDriverDob().getTime());
        ...
        ps.setString(colPos++, CacheTrack.standardizeName(biomatric_bean.getDriverName()));
        ps.setString(colPos++, CacheTrack.standardizeName(biomatric_bean.getDriverDlNumber()));
    ...
    public static boolean updateDriverFingure(Connection invConn, BioMatricBean biomatric_bean) throws Exception{
        System.out.println("#### GateInDao: updateDriverDetail() #####");
        boolean insertsuccess = false;
        PreparedStatement ps = null;
        int colPos = 1;
    ...
    public static boolean insertDriverDetail(Connection invConn, BioMatricBean biomatric_bean) throws Exception{
        System.out.println("#### GateInDao: updateDriverDetail() #####");
        boolean insertsuccess = false;
        PreparedStatement ps = null;
        int status = 1;
        ...
        try {
			...
			if (rs.next()) {
				...
				System.out.println("Generated Driver ID: " + Integer.toString(biomatric_bean.getDriverId()));
			}
            insertsuccess = true;
        } catch (Exception ex) {
            ex.printStackTrace();
...
//    public static RegistrationStatus FetchDataFromRegistrationStatus(Connection invConn, int vehId_glob) {
//
//        PreparedStatement ps = null;
//        boolean isExist = false;
//        boolean isInsert = false;
...
//                .getTime());
//        boolean destroyIt = false;
//        String Query = "Select tpr_id, tag_info, vehicle_info, driver_info, challan_record_info, multiple_tpr_info, driver_id, vehicle_id  From registration_status_rfid where vehicle_id = ?";
//        //  String updateQuery = "UPDATE registration_status_rfid SET tpr_id = ?,tag_info = ?,vehicle_info = ?,driver_info = ?,challan_record_info = ?,multiple_tpr_info = ?,updated_on = ?,vehicle_id = ? WHERE vehicle_id = ?";
...
//    }
    ...
    public static RegistrationStatus SelectFromRegistrationStatus(Connection invConn, int vehId_glob) throws Exception {
        System.out.println(" ######## Start Get SelectFromRegistrationStatus(Connection invConn, int vehId_glob) ######");
        PreparedStatement ps = null;
        boolean isExist = false;
        boolean isInsert = false;
        ResultSet rs = null;
        ...
        if (!isExist) {
            ...
            System.out.println(" ######## End Get SelectFromRegistrationStatus(Connection invConn, int vehId_glob) ######");
        }
        if (isExist) {
            try {
                regisBean.setVehicle_id(vehId_glob);
    ...
    public static Triple<Token, TPRecord, TPRBlockManager> getTPRecord(Connection conn, String vehicleName, int vehicleId, int workStationType, int workStationTypeId, int userId) {
        System.out.println(" ######## Start Get getTPRecord(Connection conn, String vehicleName, int vehicleId) ######");
        ...
        Triple<Token, TPRecord, TPRBlockManager> tpRecord = rfidProcessor.getTprecord(vehicleName, vehicleId, false,false);
        if (tpRecord != null) {
...
//
//        String query = " SELECT status FROM driver_details WHERE vehicle_id  = ? and  challan_date = ? ";
//        try {
...
//    }
//    public synchronized TPRecord syncTPRecord(String vehicleName, String challanDate) throws IOException {
//        Connection invConn = null;
//        boolean destroyIt = false;
//        PreparedStatement ps = null;
//        ResultSet rs = null;
//        //int tpr_id = Misc.getUndefInt() ;
//        TPRecord mBean = null;
//        String query = " SELECT tpr_id,transporter_id,challan_date FROM tp_record WHERE vehicle_Name  = ? and  challan_date = ? ";
//        try {
//            invConn = DBConnectionPool.getConnectionFromPoolNonWeb();
//            ps = invConn.prepareStatement(query);
...
//        //int tpr_id = Misc.getUndefInt() ;
//
//        String query = "SELECT status_result from tpr_block_detail where tpr_id = ? order by created_on ";
//        try {
//            invConn = DBConnectionPool.getConnectionFromPoolNonWeb();
//            ps = invConn.prepareStatement(query);
//            ps.setInt(1, tpr_id);
...
//        return status;
//    }
    public static Biometric getDriverDetail(Connection invConn, String driver_id) throws Exception {
//        Connection invConn = null;
        boolean destroyIt = false;
        PreparedStatement ps = null;
        ResultSet rs = null;
...
//        }
    ...
    public static Biometric insertDriverDetailByDL(Connection conn, String dlNo, String driverName, int vehId) throws Exception {
...
//            invConn = DBConnectionPool.getConnectionFromPoolNonWeb();
    ...
    public static Biometric getDriverDetailByDL(Connection conn, String dlNo, String searchStr) throws Exception {
        ...
        Biometric driverBean = null;
        String query = "";
//        String query = " SELECT id, driver_name, driver_dl_number, type, driver_photo, is_finger_captured FROM driver_details WHERE driver_dl_number = ? ";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                driverBean.setIsFingerInDB(Misc.getRsetInt(rs, "is_finger_present"));
    ...
    public static BioMatricBean getDriverDetails(Connection invConn, String dlNo, String searchStr) throws Exception {
//        Connection invConn = null;
        String std_name = CacheTrack.standardizeName(dlNo);
        boolean destroyIt = false;
        PreparedStatement ps = null;
...
//            invConn = DBConnectionPool.getConnectionFromPoolNonWeb();
	...
	public static void initManualEntry(Connection conn,boolean forceManual) throws Exception {
        PreparedStatement ps = null;
        ResultSet rs = null;
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                TokenManager.isManualEntry.put(rs.getInt(1), TokenManager.forceManual ? 1 : rs.getInt(2));
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            throw ex;
...
//		Date dayAfter = new Date(someDate.getTime()+(24*60*60*1000));		
	...
	private static Pair<String, String> identifyUser( ArrayList<byte[]> fingerPrintTemplateList){
		...
		try{
        	...
        	System.out.println("Result:"+(matchResult == null? "" : matchResult.first+","+matchResult.second));
	...
	public static void main(String s[]){
		...
		try {
			conn = DBConnectionPool.getConnectionFromPoolNonWeb();
			 ArrayList<Pair<Integer, ArrayList<byte[]>>> templateList = getDriverDebugData(conn);	
			 for(Pair<Integer,ArrayList<byte[]>> userTemplate : templateList){
				 System.out.println("Count: "+count++);
				 if(userTemplate == null || userTemplate.second == null || Misc.isUndef(userTemplate.first))
					 continue;
				 Pair<String, String> result = identifyUser(userTemplate.second);
				 int driverId = result == null ? Misc.getUndefInt() : Misc.getParamAsInt(result.first);
	...
	private static void updateIdentifyDebugDriver(Connection conn,
			int tprId, int driverId) throws Exception{
		PreparedStatement ps = null;
		try{
