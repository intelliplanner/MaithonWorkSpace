...
import com.ipssi.rfid.ui.DisconnectionDialog;
 ...
 class ThreadMonitor implements Runnable {
     ...
     public static Thread start(final Thread thread, final long timeout,final InterruptListener _listener) throws Exception{
         Thread monitor = null;
         if (timeout > 0) {
             final ThreadMonitor timout = new ThreadMonitor(thread, timeout,_listener);
     ...
     private ThreadMonitor(final Thread thread, final long timeout,final InterruptListener _listener) {
         this.thread = thread;
     ...
     public static void main(String[] arg){
    	 ...
    	 while(true){
    		 try {
    			 ThreadMonitor.stop(monitor);
    			 monitor = ThreadMonitor.start(timeoutInMillis,null);
