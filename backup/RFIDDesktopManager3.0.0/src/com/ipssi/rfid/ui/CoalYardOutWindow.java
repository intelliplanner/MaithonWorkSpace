            ...
            if (tprRecord != null) {
                if (true) {//blockStatus == Status.VALIDATION.NO_ISSUE) {// TPR Valid
                    ...
                    if (gpsRepair.first == ResultEnum.GREEN) {
                        ...
                        /*if(TPRBlockStatusHelper.useForBlocking(Status.TPRQuestion.gpsOk) && tprRecord.getMarkForGPS() == 1){
                        	Pair<Integer, Boolean> gpsBlockStatus = TPRBlockStatusHelper.isBlockedForInstruction(conn, tprRecord.getTprId(), Type.BlockingInstruction.BLOCK_DUETO_GPS);
                        	if(Misc.isUndef(gpsBlockStatus.first) || gpsBlockStatus.second){
                        		enableDenyEntry(true);
                        		blocking_reason.setText(RFIDDataProcessor.getTprBlockStatus(tprRecord, blockInstructionGps, true));
                        	}
                        }*/
                    } else {
