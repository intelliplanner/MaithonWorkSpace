...
public class MorphoEventHandler implements IMorphoEventHandler {
	...
	public MorphoEventHandler(MorphoActionI morphoActionI) {
	...
	public void onCommandStatusEvent(MorphoCommandStatus fingerStatus) {
		...
		case MORPHO_REMOVE_FINGER:
			...
			if (MorphoInfo.isFVP)
				isFingerOk = true;
			morphoActionI.setBorderColorGreen(fingerNumber, step);
			break;
		...
		morphoActionI.setInstruction(intruction);
		morphoActionI.setStepsImage(move);
		if (MorphoInfo.isFVP) {
			if (isFingerOk)
				morphoActionI.fingerOk();
			else
				morphoActionI.playVideo(isFingerFvpDetected);
		}
	}
	...
	public void onImageEvent(MorphoImage morphoImage) {
		if (morphoImage != null) {
			...
			morphoActionI.setLiveStepImage(morphoImage, fingerNumber, step);
			if (!MorphoInfo.isFVP) {
				morphoActionI.setLiveImage(morphoImage);
			}
			...
			morphoActionI.setCurrentImageInfo(nbCol, nbRow,
	...
	public void onCodeQualityEvent(short quality) {
		morphoActionI.setCodeQuality(quality);
		morphoActionI.setScore(quality);
	}

	@Override
	public void onDetectQualityEvent(short quality) {
		morphoActionI.setDetectedQuality(quality);
		morphoActionI.setScore(quality);
	}
