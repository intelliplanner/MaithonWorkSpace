 ...
 * @author Vi$ky
 */
public class FlyAshGrossSlip extends javax.swing.JDialog {
...
//    private String grade = "";
    ...
    private String mplRefDocNo = "";
    private String material = "Ash";//FlyAsh
    private String materialSubCat = "";

    public FlyAshGrossSlip(java.awt.Frame parent, boolean modal, TPRecord tpRecord, String materialSubCat) {
        super(parent, modal);
        initComponents();
...
//                                StyleConstants.setAlignment(label, StyleConstants.ALIGN_RIGHT);
        ...
        try {
            ...
            doc.insertString(doc.getLength(), getLabelString("Invoice#:", 16), label);
            ...
            doc.insertString(doc.getLength(), getLabelString("Invoice Date:", 16), label);
            ...
            doc.insertString(doc.getLength(), getLabelString("Order#:", 16), label);
            doc.insertString(doc.getLength(), getString(mplRefDocNo, 20), text);
            doc.insertString(doc.getLength(), getLabelString("LR#:", 16), label);
            ...
            doc.insertString(doc.getLength(), getLabelString("Consignee:", 16), label);
            doc.insertString(doc.getLength(), Misc.getPrintableString(consignee) + "\n", text);


            doc.insertString(doc.getLength(), getLabelString("Transporter#:", 16), label);
            doc.insertString(doc.getLength(), getString(tranporter, 55) + "\n", text);

            doc.insertString(doc.getLength(), getLabelString("Value of Goods:", 16), label);
            doc.insertString(doc.getLength(), getString("NIL", 9), label);
            doc.insertString(doc.getLength(), getLabelString("Category:", 27), label);
            ...
            doc.insertString(doc.getLength(), getLabelString("", 10), label);
...
//            doc.insertString(doc.getLength(), getLabelString("Tare WB User", 20), label);
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                FlyAshGrossSlip dialog = new FlyAshGrossSlip(new javax.swing.JFrame(), true, null, null);
    ...
    private void initializeVariables() {
        if (tpRecord != null) {
            ...
            try {
                ...
                if (tpRecord.getLatestLoadWbOutExit() != null) {//setLatestLoadWbOutExit
                    grossTime = UIConstant.displayFormat.format(tpRecord.getLatestLoadWbOutExit());
                }

                tranporter = DropDownValues.getTransporterForSlip(tpRecord.getTransporterId(), conn);

                vehicleNo = tpRecord.getVehicleName();

                challanNo = tpRecord.getChallanNo();
                lrNo = tpRecord.getConsigneeRefDoc();
                ...
                double net_Wt = GateInDao.calculateNetWt(tpRecord.getLoadGross(), tpRecord.getLoadTare());
                //netWts = Misc.getPrintableDouble(net_Wt < 16.5 ? net_Wt : 16.5);                
                netWts = Misc.getPrintableDouble(net_Wt);
                preparedBy = TokenManager.userName;
                checkedBy = "";
                mplRefDocNo = Misc.getPrintableString(tpRecord.getMplReferenceDoc());

                materialSubCat = Utils.isNull(materialSubCat) ? "" : "(" + materialSubCat + ")";
                material += materialSubCat;
...
//
//    private String getMaterialSubCatName(Connection conn, int material_id) {
//        PreparedStatement ps = null;
//        String materialName = "";
//        ResultSet rs = null;
//        int parameterIndex = 1;
//        String query = "SELECT name FROM material_sub_cat WHERE id = ?";
//        try {
//            ps = conn.prepareStatement(query);
//            ps.setInt(parameterIndex++, material_id);
//            rs = ps.executeQuery();
//            while (rs.next()) {
