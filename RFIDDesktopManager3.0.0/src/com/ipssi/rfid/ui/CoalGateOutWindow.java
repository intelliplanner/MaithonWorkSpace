...
public class CoalGateOutWindow extends javax.swing.JFrame {
	...
	private void setTPRecord(Connection conn, String vehicleName)
			throws IOException {
		if (rfidHandler != null) {
			...
			// (tpRecord.second, tpRecord.third);
			// }
		}
	}
		...
		try {
			if (TokenManager.isBlockedVehicle) {
				JOptionPane.showMessageDialog(null, "Vehicle '"
						+ tpr.getVehicleName()
						+ "' is Blocked for GATE-IN/GATE-OUT ");
				return;
			}
			tprRecord = tpr;
			if (tprRecord != null) {
				...
				if (true) {// blockStatus == Status.VALIDATION.NO_ISSUE) {// TPR
					...
					if (token == null
							&& tprRecord.getEarliestUnloadGateOutEntry() != null) {
					...
					vehicleName.setText(tprRecord.getVehicleId(), tprRecord
							.getVehicleName());
					challanNo.setText(tprRecord.getChallanNo());
					transporter.setText(DropDownValues.getTransporter(tprRecord
							.getTransporterId(), conn));
					purpose.setText(com.ipssi.rfid.constant.Type.TPRMATERIAL
							.getStr(tprRecord.getMaterialCat()));
				}

				pairVal = GpsPlusViolations.getLatestLocation(conn, tprRecord
						.getVehicleId());
				if (pairVal != null) {
	...
	private void initComponents() {
		...
		this.addWindowListener(new java.awt.event.WindowAdapter() {
			...
			public void windowClosing(java.awt.event.WindowEvent windowEvent) {
				GateInDao.forceSignut(TokenManager.userId,
						TokenManager.srcType, Integer
								.toString(TokenManager.systemId));
			}
		});
		 ...
		 * vehicleName.setText(" NO VEHICLE DETECTED");
		 * vehicleName.setBorder(null); vehicleName.setFocusable(false);
		 * vehicleName.addMouseListener(new java.awt.event.MouseAdapter() {
		 * public void mouseClicked(java.awt.event.MouseEvent evt) {
		 * vehicleNameMouseClicked(evt); } }); vehicleName.addFocusListener(new
		 * java.awt.event.FocusAdapter() { public void
		 * focusGained(java.awt.event.FocusEvent evt) {
		 * vehicleNameFocusGained(evt); } public void
		 * focusLost(java.awt.event.FocusEvent evt) { vehicleNameFocusLost(evt);
		...
		vehicleName.setModel(new javax.swing.DefaultComboBoxModel(
				new String[] { "Select" }));
		vehicleName.setTextBorder(javax.swing.BorderFactory.createLineBorder(
		...
		jPanel1Layout.setHorizontalGroup(jPanel1Layout.createParallelGroup(
				...
				jPanel1Layout.createSequentialGroup().addComponent(jPanel4,
						javax.swing.GroupLayout.PREFERRED_SIZE, 1356,
	...
	private void button1ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_button1ActionPerformed
		stopRfid();
		GateInDao.forceSignut(TokenManager.userId, TokenManager.srcType,
				Integer.toString(TokenManager.systemId));
		this.dispose();
		try {
	...
	// && !vehicleName.getText().equalsIgnoreCase("NO VEHICLE DETECTED")){
	// vehicleNameAction();
	// }
	...
	public static void main(String args[]) {
		...
		try {
			...
			java.util.logging.Logger.getLogger(
					CoalGateOutWindow.class.getName()).log(
		...
		java.awt.EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					Logger.getLogger(CoalGateOutWindow.class.getName()).log(
	...
	/*
	 * private void toggleVehicle(boolean editable) { if(editable){
	 * vehicleName.setEnabled(true); vehicleName.setEditable(true);
	 * vehicleName.setFocusable(true); vehicleName.requestFocusInWindow();
	 * vehicleName.setTextBackground(UIConstant.focusPanelColor);
	 * vehicleName.setText("");
	 * //vehicleName.setBorder(javax.swing.BorderFactory.createLineBorder(new
	 * java.awt.Color(0, 0, 0))); }else{ vehicleName.setEnabled(false);
	 * vehicleName.setEditable(false); vehicleName.setBorder(null);
	 * vehicleName.setTextBackground(Color.WHITE); } //
	 * transporter.setFocusable(true); // transporter.setEnabled(true); }
	 */
	private void toggleVehicle(boolean editable) {
		if (editable) {
			...
			vehicleName.setTextBackground(UIConstant.focusPanelColor);
			vehicleName.setText("");
			vehicleName.setTextBorder(javax.swing.BorderFactory
	...
	private void updateTPR(Connection conn, boolean isDeny) throws Exception {
		if (!isDeny) {
			...
			if (tprRecord.getMaterialCat() == com.ipssi.rfid.constant.Type.TPRMATERIAL.FLYASH
					|| tprRecord.getMaterialCat() == com.ipssi.rfid.constant.Type.TPRMATERIAL.STONE
					|| tprRecord.getMaterialCat() == com.ipssi.rfid.constant.Type.TPRMATERIAL.OTHERS) {
	...
	private int InsertTPRStep(Connection conn, boolean isDeny,
			boolean repeatProcess) throws Exception {
		if (tpStep == null || Misc.isUndef(tpStep.getId())) {
			...
			// tpStep.setCoalGateOutGpsLocation(GateInDao.removeSpecialChar(gpsLocation.getText()));
			tpStep.setGpsLocation((gpsLocation.getText() != null && gpsLocation
					.getText().trim().length() > 0) ? GateInDao
					.removeSpecialChar(gpsLocation.getText()) : "");
			tpStep.setGpsRecordTime((pairVal != null && !Misc
					.isUndef(pairVal.first)) ? new Date(pairVal.first) : null);
			if (repeatProcess)
	...
	private void changeToQuickCreateMode() {
		...
		vehicleName.setTextBackground(UIConstant.focusPanelColor);
		vehicleName.setText("");
		vehicleName.setBorder(javax.swing.BorderFactory
				.createLineBorder(new java.awt.Color(0, 0, 0)));
	}
	...
	private void quickCreateAndSave() {
		...
		try {
			...
			if (isRequestOverride) {
				...
				if (vehicleName.isTextEditable()
						&& Utils.isNull(vehicleName.getText())) {
				...
				Triple<TPRecord, Integer, Boolean> tprTriplet = TPRInformation
						.getLatestNonWeb(conn, token.getVehicleId(), null,
								vehicleName.getText(),
								TokenManager.currWorkStationType,
								TokenManager.materialCat);// (conn, vehicleId,
															// data,
															// vehicleName,
				...
				if (tpr != null
						&& TPRInformation.isGreaterThanEqualsProcessed(tpr,
								TokenManager.currWorkStationType,
				...
				if (!repeatProcess)
					updateTPR(conn);
				InsertTPRStep(conn, false, repeatProcess);
				conn.commit();
	...
	private void vehicleNameAction() {
		...
		try {
			...
			if (Utils.isNull(vehicleName.getText())) {
				...
				vehicleName.setText(CacheTrack.standardizeName(vehicleName
						.getText()));
				Pair<Integer, String> vehPair = TPRInformation.getVehicle(conn,
						null, vehicleName.getText());
				if (vehPair != null) {
					vehId = vehPair.first;
				}
				...
				if (!isVehicleExist) {
					JOptionPane.showMessageDialog(null,
							"Invalid Vehicle,please enter valid vehicle");
					/*
					 * Object[] options = {"  Re-Enter  ", "  Continue  "};
					 * String msg =
					 * "Invalid Vehicle Please Go to Registration Office"; int
					 * responseVehicleDialog =
					 * ConfirmationDialog.getDialogBox(new javax.swing.JFrame(),
					 * true, options, msg); if (responseVehicleDialog ==
					 * reEnter) { setWhiteBackColor(); vehicleName.setText("");
					 * vehicleName
					 * .setTextBackground(UIConstant.focusPanelColor); return; }
					 * else if (responseVehicleDialog == contiNue) {
					 * setWhiteBackColor(); vehicleName.setText("");
					 * vehicleName.
					 * setTextBackground(UIConstant.focusPanelColor); return; }
					 */
				} else {
