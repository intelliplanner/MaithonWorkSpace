...
public class TokenManager {
	...
	public static final String ACTION = "GET_GPS_VIOLATIONS";
	public static final String URL = "http://203.197.197.16:8980/LocTracker/getGpsViolationDetails.jsp";
...
//	 HashMap<Integer,String> hm=new HashMap<Integer,String>();  
	...
	public static boolean isSingleSignOn=false;
	public static HashMap<String,String> blockedVehicle = null;
	public static boolean isBlockedVehicle = false;
	public static final String IMAGE_BASE = "/com/images/";
	public static boolean isTokenAvailable(Connection conn, String epcId) {
		return isTokenAvailable(conn, epcId,null);
	}
	public static boolean isTokenBlocked(Connection conn, String epcId) {
		if(blockedVehicle == null){
			GateInDao.loadBlockedVehicle(conn,epcId);
		}
		
		if(blockedVehicle!=null && blockedVehicle.size()>0){
			if(blockedVehicle.containsKey(epcId))
				isBlockedVehicle=true;
			else
				isBlockedVehicle=false;
		}
		return isBlockedVehicle;
	...
	public static void initSystemConfig(){
		try{
			...
			Misc.g_doSingleSession = Misc.getParamAsInt(config.getProperty("MISC_DO_SINGLE_SESSION")) == 1;
		}catch(Exception ex){
			ex.printStackTrace();
		}
	}
	public static void initConfig(String sufix,int workStationType){
		try{
			...
			if(config != null){
				...
				screenClearInterval = Misc.getParamAsLong(config.getProperty("SCREEN_CLEAR_INTERVAL"),180);
				TPRInformation.setSameStationTprThresholdMinutes(sameStationThreshold);
				isIssueSampleTag = Misc.getParamAsInt(config.getProperty("ISSUE_SAMPLE_TAG")) == 1;
