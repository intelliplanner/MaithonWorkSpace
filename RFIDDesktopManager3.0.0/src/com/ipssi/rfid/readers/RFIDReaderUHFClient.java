...
public class RFIDReaderUHFClient implements ReaderI {
	...
	public static void main(String[] args) {
		...
		for (int i = 0, is = tags == null ? 0 : tags.size(); i < is; i++) {
			RFIDTagInfo tag = rf.getData(Utils
					.HexStringToByteArray(tags.get(i)));
	...
	/*
	 * ReadCard_G2 (): Function description: The function is used to read part
	 * or all of a Tag’s Password, EPC, TID, or User memory. To the word as a
	 * unit, start to read data from the designated address. Usage: int[]
	 * ReadCard_G2(int[]arr); Parameter: arr: Input array. First byte is the
	 * address of the reader Second is the byte length of EPC. Third to
	 * EPC-length+2 is EPC number. EPC-length+3th is select the memory area to
	 * read. 0x00: Password area; 0x01: EPC memory area; 0x02: TID memory area;
	 * 0x03: User’s memory area; Other value when error occurred. EPC-length+4th
	 * is the address of tag data to read (Word/Hex). Such as, 0x00 stand in
	 * start to read data from first word, 0x01 stand in start to read data from
	 * second word, and so on. EPC-length+5th is the number of word to read. Can
	 * not set 0 or 120, otherwise, return the parameter error information. Num
	 * <= 120 EPC-length+6th to EPC-length+9th is Password. EPC-length+10th is
	 * EPC masking starting address of byte. EPC-length+11th is Masking bytes.
	 * EPC-length+12th is EPC masking Flag. 0x00:disabled; 0x01:enabled;
	 * EPC-length+13th is the handle of port. Returns: Return an array ,first is
	 * non-zero value when error occurred.Then second byte is error code. Zero
	 * value when successfully,then other byte is the Read data
	 */
	@Override
	public RFIDTagInfo getData(byte[] epc) {
		...
		if (reader == null || Misc.isUndef(handle) || epc == null
				|| epc.length < 12)
			return null;
		int[] data = new int[25];
		int i = 0;
