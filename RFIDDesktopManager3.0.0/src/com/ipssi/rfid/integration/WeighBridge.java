				...
				public void run() {
					...
					while(isConnected()){
						try {
							...
							monitor = ThreadMonitorOld.start(TokenManager.weighBridgeTimeout,new InterruptListener() {
								@Override
	...
	public void startWeighBridgeEventBased(){
		try
		{
			if(isConnected()){
				...
				if(listener != null)
					listener.removeDisconnection();
				monitor = ThreadMonitorOld.start(TokenManager.weighBridgeTimeout*10, new InterruptListener() {
